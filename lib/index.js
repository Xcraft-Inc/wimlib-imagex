  ...
  *_spawn(action, ...args) {
  ...
  static _argsOptionsBuilder(options) {
    ...
    if (options.sourceList) {
      args.push(`--source-list=${options.sourceList}`);
    }
    if (options.compress) {
   ...
   */
  async capture(outputWim, sourceDir = null, options = {}) {
    if (!options) {
      options = {};
    }
    if (!sourceDir && !options.sourceList) {
      throw new Error('Set a sourceDir or a sourceList via the options');
    }

    const args = Wim._argsOptionsBuilder(options);
    args.unshift(outputWim);
    if (sourceDir) {
      args.unshift(sourceDir);
    }
    await this._spawn('capture', sourceDir, outputWim, ...args);
  }
